...
export default function RichTextEditor({ content, onChange, placeholder = "Viết nội dung..." }: RichTextEditorProps) {
    ...
    return (
        <div className="border-t border-gray-100 relative">
            {/* Toolbar */}
            <div className="flex flex-wrap items-center gap-0.5 px-4 py-2 border-b border-gray-100 bg-gray-50/50 sticky top-0 z-10 rounded-t-lg">
                <ToolbarButton 
                    onClick={() => {
                        setIsHtmlMode(!isHtmlMode);
                        // Khi chuyển từ Raw HTML sang Visual Mode, bắt Editor render lại mớ HTML mới nhất
                        if (isHtmlMode) {
                            editor.commands.setContent(content);
                        }
                    }} 
            ...
            {isHtmlMode ? (
                <textarea
                    ...
                    className="w-full min-h-[500px] p-6 lg:p-10 text-sm font-mono text-gray-800 bg-gray-50 border-none outline-none focus:outline-none focus:ring-0 resize-y"
                ...
                <div className="p-6 lg:p-10 min-h-[500px] prose-editor-wrapper bg-white">
